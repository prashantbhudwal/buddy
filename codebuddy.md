...
## 4.2 `packages/buddy/src/agent/agent.ts`

This file defines Buddy's local agent catalog.

The main change is the addition of a new primary agent:

- `code-teacher`

### `code-teacher` properties

- `name`: `code-teacher`
- `mode`: `primary`
- `native`: `true`
- `steps`: `8`
- `prompt`: loaded from `packages/buddy/src/session/prompts/code-teacher.txt`

### `code-teacher` permissions

The agent starts from the same permissive defaults as the normal `build` agent, then adds or overrides:
...
Then, after merging user config, it force-denies:
...
- the teaching agent previously drifted into internal todo tracking,
- that caused fake progress advancement,
- the hard deny prevents the teacher from using todo tools as a proxy for learner progress.

### Runtime enforcement note

`packages/buddy/src/agent/agent.ts` is Buddy's product-layer catalog and UI-facing definition.

The actual vendored runtime now also receives a matching `code-teacher` definition through the adapter config overlay, so the OpenCode runtime resolves the same agent name directly during prompt execution.
