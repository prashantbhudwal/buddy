...
function abortAfterAny(timeoutMs: number, signal: AbortSignal) {
  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort("webfetch timeout"), timeoutMs)
  const onAbort = () => controller.abort("request aborted")
    ...
    try {
      ...
      const response =
        initial.status === 403 && initial.headers.get("cf-mitigated") === "challenge"
          ? await fetch(url, { signal, headers: { ...headers, "User-Agent": "buddy" } })
          : initial
