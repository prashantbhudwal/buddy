...
function resolveMaxLoopSteps() {
  const raw = process.env.SESSION_MAX_STEPS
  if (!raw) return DEFAULT_MAX_LOOP_STEPS
  const parsed = Number.parseInt(raw, 10)
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return DEFAULT_MAX_LOOP_STEPS
  }
  return Math.min(parsed, MAX_LOOP_STEPS_HARD_CAP)
}
